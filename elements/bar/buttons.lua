	...
	local BarPositions = {
		['dksdBar1'] = {
			STACKED = {'BOTTOM', 		UIParent, 'BOTTOM',   0, 	60},
		...
		['dksdBar2'] = {
			STACKED = {'BOTTOM', 		UIParent, 'BOTTOM',   0, 	96},
		...
		['dksdBar3'] = {
			STACKED = {'BOTTOM', 		UIParent, 'BOTTOM',   0, 	140},
	...
	-- 	ExtraActionBarFrame:SetPoint('BOTTOM', UIParent, 0, 240)
	...
	-- 	-- 	ActionBarUpButton:SetPoint('TOPRIGHT', _G['dksdBar1'], 'TOPLEFT', -20, 5)
	...
	-- 	-- else
	-- 	-- 	ActionBarUpButton:SetPoint('TOPLEFT', _G['dksdBar1'], 'TOPRIGHT', 30, 5)
	-- 	-- 	ActionBarDownButton:SetPoint('TOP', ActionBarUpButton, 'BOTTOM', 0, 14)
	-- 	-- 	MainMenuBarPageNumber:SetPoint('TOPRIGHT', ActionBarUpButton, 'BOTTOMLEFT', -2, 9)
	-- 	-- end
	...
	-- 	-- local bu = CreateFrame('CheckButton', 'dksdVehicleExitButton', UIParent, 'ActionButtonTemplate, SecureHandlerClickTemplate')
	-- 	-- bu:SetSize(21, 21)
	...
	-- 		local list = GetButtonList('ActionButton', 11)
	-- 	    if  InCombatLockdown() then
	-- 	      	dksd:RegisterEvent('PLAYER_REGEN_ENABLED', 	 AddFloatingGrid)
	-- 	      	return
	-- 	    end
	-- 	    local var = tonumber(GetCVar'alwaysShowActionBars')
	-- 		dksd:UnregisterEvent('PLAYER_REGEN_ENABLED',		 AddFloatingGrid)
	...
	local AddBars = function()					-- 1:  action 	2:  multileft 	3:  multiright
		for i = 1, 7 do							-- 4:  right	5:  left	6:  stance	7: pet
			local barDatas = BarPositions['dksdBar'..i]
			local x, y = _G['ActionButton1']:GetSize()
			local bar  = CreateFrame('Button', 'dksdBar'..i, f, 'SecureHandlerStateTemplate')
			bar:SetFrameStrata("HIGH")
			bar:SetFrameLevel(4)
			bar:SetSize(x * barDatas.length + 7.3 * barDatas.length, y)
			add(bar)

			local v = BarPositions['dksdBar'..i].visibility
	...
	local ResizeBars = function ()
		for idx, name in pairs(
			...
			local barDatas = BarPositions['dksdBar'..idx]
			local x, y = _G['ActionButton1']:GetSize()
			local bar = _G['dksdBar'..idx]
			bar:SetSize(x * barDatas.length + 7.3 * barDatas.length, y)
			for j = 1, barDatas.maxLength do
				if j > barDatas.length then 
					local bu = _G[name..j]
					bu:Hide()
				end
			end
	...
	local AddButtonsToBar = function(num, max, name, parent, old, page)
		if  old then
		...
		for i = 1, max do
			...
			if (i > num) then 
				...
				if  i == 1 then
					...
					-- print(_G[name..i - 1])
					bu:SetPoint('LEFT', _G[name..i - 1], 'RIGHT', name == 'StanceButton' and 11 or name == 'PetActionButton' and 6 or 8, 0)
					-- bu:SetPoint('LEFT', _G[name..i - 1], 'RIGHT', 0)
				end
				...
				if  name == 'StanceButton' then
					...
					if  StanceBarFrame:IsShown() then
						_G['dksdBar6']:Show() bu:Show()
					else
						_G['dksdBar6']:Hide() bu:Hide()
					end
				end
	...
	-- 	bu:ClearAllPoints()
	-- 	bu:SetPoint('BOTTOM', _G['dksdBar6'], i > 1 and -(20*i) + 16 or 0, 0)
	-- end
	...
	-- 	dksd:UnregisterEvent('PLAYER_REGEN_ENABLED', PositionBarsAfterCombat)
	...
	local SetUpButtons = function()
		...
		AddButtonsToBar(11,	12, 'ActionButton',					_G['dksdBar1'], MainMenuBarArtFrame)
		AddButtonsToBar(11, 12, 'MultiBarBottomLeftButton',		_G['dksdBar2'], MultiBarBottomLeft, 6)
		AddButtonsToBar(7, 12, 'MultiBarBottomRightButton',	_G['dksdBar3'], MultiBarBottomRight, 5)
		AddButtonsToBar(11, 12, 'MultiBarRightButton',			_G['dksdBar4'], MultiBarRight, 4)
		AddButtonsToBar(11, 12, 'MultiBarLeftButton',			_G['dksdBar5'], MultiBarLeft, 3)
		AddButtonsToBar(10, 10, 'StanceButton',					_G['dksdBar6'], StanceBarFrame)
		AddButtonsToBar(10, 10, 'PetActionButton',				_G['dksdBar7'], PetActionBarFrame)
		-- AddStancePositions()

		dksd:RegisterEvent('ACTIONBAR_SHOWGRID', ResizeBars)
		...
		hooksecurefunc('MultiActionBar_Update', function()
			if InCombatLockdown() then
				dksd:RegisterEvent('PLAYER_REGEN_ENABLED', PositionBarsAfterCombat)
			else
				C_Timer.After(1.5, SetPositions)
			end
		end)
	...
	-- 	ShowBar(_G['dksdBar7'])
	-- end)
	SpellBookFrame:HookScript('OnShow', ResizeBars)
	SpellBookFrame:HookScript('OnHide', ResizeBars)
