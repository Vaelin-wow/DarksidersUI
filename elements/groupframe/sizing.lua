    ...
    if not dksdRaidX then
		dksdRaidX = 50
		dksdRaidY = 35
	end

	ns.CreateSizingFrame = function()
		--[[local sizer = CreateFrame('Button', 'dksdraidsizer', oUF_partyUnitButton1.Health)
		sizer:SetSize(16, 16)
		...
		local UpdateFrames = function()
			for i = 1, 40 do
				...
				if 	bu then
					...
					bu:SetSize(50, 35)
					dksdRaidX, dksdRaidY = 50, 35	-- save as cvar
				end
			end
			for i = 1, 8 do
				local bu = _G['dksdRaidGroupNo'..i]
				if bu then
					if i == 1 then
						bu:SetPoint('BOTTOMLEFT', ChatFrame1, 'TOPLEFT', 0, 50 + (oUF_partyUnitButton1:GetHeight() - 5))
					else
						bu:SetPoint('BOTTOMLEFT', _G['dksdRaidGroupNo'..(i - 1)], 'TOPLEFT', 0, oUF_partyUnitButton1:GetHeight())
					end
				end
			end
		end
