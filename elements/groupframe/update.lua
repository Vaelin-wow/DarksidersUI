    ...
    local OverrideLFD = function(self, override)
		...
		if  LFDRole then
            ...
            -- else
            if role == 'HEALER' then
				LFDRole:SetTexture[[Interface\AddOns\DarksidersUI\art\unitframe\role\Healer.tga]]
				-- LFDRole:SetTexCoord(0, 1, 0, 1)
				LFDRole:Show()
			elseif role == 'TANK' then
				LFDRole:SetTexture[[Interface\AddOns\DarksidersUI\art\unitframe\role\Tank.tga]]
				-- LFDRole:SetTexCoord(0, 1, 0, 1)
				LFDRole:Show()
			else
				LFDRole:SetTexture''
    ...
    local UpdateName = function(self, unit, show)
        ...
        -- self.Name:SetTextColor(colour.r, colour.g, colour.b)
        -- self.Name:Hide()
	...
	local UpdateGroupIndicator = function()
		for i = 1, 6 do
			_G['dksdRaidGroupNo'..i]:Hide()
		end
		for i = 1, GetNumGroupMembers() do
			local _, _, subgroup = GetRaidRosterInfo(i)
			for j = 1,  subgroup do
				if  _G['dksdRaidGroupNo'..j] then
                    local GroupNo = _G['dksdRaidGroupNo'..j]
					GroupNo:SetText(j)
					GroupNo:Show()
				end
			end
