    ...
    local UpdateSB = function(self, duration, expiration)
        ...
        if  expiration and duration > 0 then
            ...
            if self.debuff then 
                ...
                self.sb:SetStatusBarColor(colour.r, colour.g, colour.b)
            end
    ...
    local AddButton = function(self, name, bu)
        ...
        bu.bg = bu:CreateTexture('$parentTexture', 'BACKGROUND')
        bu.bg:SetTexture[[Interface\AddOns\DarksidersUI\art\misc\AurasBG]]
        bu.bg:SetSize(51, 55)
        bu.bg:SetPoint('TOPLEFT', bu, -7, 5)
        ...
        local name = bu:CreateFontString('$parentName', nil, 'dksdAuraFont')
        ...
        d:SetPoint('TOP', bu, 'BOTTOM', 0, -26)
        d:SetTextColor(1,1,1,1)
        d:SetJustifyH'CENTER'

    	local count = bu:CreateFontString('$parentCount', nil, 'dksdAuraFont')
        count:SetParent(bu.F)
        count:SetJustifyH'CENTER'
        count:SetPoint('CENTER', bu, 'TOP', 0, 16)
        ...
        sb:SetPoint'LEFT'
        sb:SetPoint'RIGHT'
        sb:SetPoint('BOTTOM', 0, -5)
        sb:SetMinMaxValues(0, 1)
        sb:SetStatusBarTexture[[Interface\AddOns\DarksidersUI\art\unitframe\Health-Target]]
        sb:Hide()
    ...
    local AddHeader = function(unit, filter, attribute)
        local Header = CreateFrame('Frame', 'dksdauras'..filter, Minimap, 'SecureAuraHeaderTemplate')
        Header:SetAttribute('template',         'darksidersauraTemplate')
        ...
        if  unit ~= 'weapons' then
            ...
            -- Header:SetAttribute('yOffset',          -75)
            Header:SetAttribute('xOffset',          attribute.x)
            -- Header:SetAttribute('yOffset',          attribute.y)
