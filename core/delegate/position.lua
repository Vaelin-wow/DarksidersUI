	...
	local DirtyUpdateBars = function()
		...
		dksd:UnregisterEvent('PLAYER_REGEN_ENABLED', 	DirtyUpdateBars)
	end

	local AssignUpdate = function(self, event)
		if  InCombatLockdown() then
			dksd:RegisterEvent('PLAYER_REGEN_ENABLED', 	DirtyUpdateBars)
		else
			if  event == 'PLAYER_ENTERING_WORLD' then
				UpdateBarsShown()
				dksd:UnregisterEvent('PLAYER_ENTERING_WORLD', AssignUpdate)
			else
				C_Timer.After(1, function()		--  run c_timer because results from GetActionBarToggles() take a second to update
					if  InCombatLockdown() then	--  sdksdped into combat since last check
						dksd:RegisterEvent('PLAYER_REGEN_ENABLED', DirtyUpdateBars)
					else
						UpdateBarsShown()
					end
	...
	dksd:RegisterEvent('PLAYER_ENTERING_WORLD', 	AssignUpdate)
